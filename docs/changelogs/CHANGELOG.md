...
### Added
- **TRP-82 (Phase A–C):** Investigation-episode segmentation + modality tag. The lifecycle-FSM `Episode` now carries an `arm` (`tp` / `shell` / `mixed` / `none`), per-episode `tp_response_tokens`, and `edit_observed_at`; `get_effectiveness_report` gains a `per_episode_cost` block reporting **cost-per-resolved-episode as `{value, n, CI}`** — `time_to_edit_ms` (effort proxy, excludes the resolution-window timer) and `tool_calls` stratified by disjoint arm, plus `tp_response_tokens` **overall-only** (shell-arm gets ~0 token attribution by design, so a by-arm token comparison would be a method artifact). Means use a **t-critical** CI with a `low_confidence` flag for small n. Arm/count stamped at the hook layer (`onErrorInvestigated`→tp, `onCommandRun`→shell on active episodes); tokens attributed best-effort by the TRP-78 middleware for the three fingerprint-bearing read tools (`verify_fix` is shell-arm, not token-attributable). New `src/store/tool-arms.ts` (single classification source), `src/analysis/episode-cost.ts` (mean-CI + per-episode metrics). Observational, not causal (TRP-85); TracePulse's own token proxy, not real agent cost (TRP-83). Spec + readiness review: `.kiro/specs/telemetry-episode-segmentation/`. Persistence (Phase D) lands separately.
- **TRP-71:** ADR-004 (`docs/decisions/ADR-004-claude-code-harness-parity.md`) records the Claude Code harness-parity decision — dogfood the shipped PreToolUse gate in TracePulse's own agent sessions and re-express the Kiro discipline hooks in Claude Code's event model, keeping each hook's original blocking-vs-advisory semantics (hard gate stays `deny`; the four `askAgent` hooks become advisory `additionalContext`). The hooks themselves live in the gitignored local `.claude/settings.json` (per TRP-39, public tree ships only `skills/`); this ADR is the tracked, auditable record and the by-hand sync checklist between the two harnesses.
